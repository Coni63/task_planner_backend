...
def _has_group(request, group: str, app_name: str = "core") -> bool:
    return request.user.has_perm(f"{app_name}.{group}")
...
class CanReadOrmQ(permissions.BasePermission):
    def has_permission(self, request, view):
        return _is_authenticated(request) and _has_group(request, "view_ormq", app_name="django_q")


class CanUpdateOrmQ(permissions.BasePermission):
    def has_permission(self, request, view):
        return _is_authenticated(request) and _has_group(request, "change_ormq", app_name="django_q")


class CanDeleteOrmQ(permissions.BasePermission):
    def has_permission(self, request, view):
        return _is_authenticated(request) and _has_group(request, "delete_ormq", app_name="django_q")


class CanCreateOrmQ(permissions.BasePermission):
    def has_permission(self, request, view):
        return _is_authenticated(request) and _has_group(request, "add_ormq", app_name="django_q")
