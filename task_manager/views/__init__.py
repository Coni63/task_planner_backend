from .categories_view import CategoryList, CategoryDetail
from .menu_view import MenuView
from .project_view import ProjectList, ProjectDetail
from .status_view import StatusList, StatusDetail
from .task_view import TaskList, TaskDetail, TaskPickView, TaskListView, TaskOrderView
from .user_assignment_view import UserAssignmentList, UserAssignmentDetail
from .user_view import UserList, UserDetail


# TODO: All Users availability
# TODO: User availability get
# TODO: User availability create
# TODO: User availability update
# TODO: User availability delete

# TODO: RememberMe